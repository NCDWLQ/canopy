...
without first expanding the sidebar. Within the sidebar, "新建对话" is a global
...
1. **Sidebar Header Toolbar**: The top `h-12` bar of `<aside>` is dedicated to
   workspace branding (`Canopy`) and the primary `新建对话` icon button (`size="icon" className="size-8"` with Tooltip).
2. **Symmetrical Section Headers**: `历史记录` and `对话树` each have their
...
3. **Collapsed Sidebar Quick Entry**: When `!isSidebarOpen`, the main window
   `<header>` renders an identical icon-only `新建对话` button (`size="icon" className="size-8"`)
   next to the sidebar expand toggle (`PanelLeftOpen`), complete with tooltip and `aria-label="新建对话"`.
